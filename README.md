...
## 한줄 회고
- **`김소희`**
> 백엔드 API를 프론트에서 검증하고, Django·FastAPI를 Docker로 표준화해 일관된 실행 환경을 만들었습니다. 개발이 진행될수록 환경 관리와 개발 확장성의 중요성을 체감했습니다. 다음에는 더욱 체계적인 시스템, 모델 설계 계획에 도전해보고 싶습니다.
- **`박준영`**
...
- **`정종현`**
> 배포에 신경쓰지 않았던 백엔드의 설정이 원할하지 않았습니다. 이는 각 연계 부분의 copy만 하다보니, 전혀 알수가 없었습니다. 이후에 프로젝트환경의 구동 구조와 검증 방법을 최대한 많이 준비하도록 하겠습니다.
